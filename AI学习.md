...
- **GPT（genertaive pretraining transformer）** Transform的Decoder生成式的结构。现在也支持理解     
...
> [!NOTE]
>
> **Q：**Embedding这个操作是做了什么？
>
> **A：** **Embedding操作**（嵌入操作）的核心目的是**将离散的符号（如单词、短语、类别标签等）转换为连续的、低维的向量表示**，也可以称为**向量化**的过程，对应上面CNN 、RNN的一些算法
>
...
- 越狱攻击
  ...
  Jailbreaking  让大模型判断错误，越过安全控制。例如：场景扮演
...
- The ASLs (AI Safety Levels)：一个评测大模型安全性的研究
...
> 蒙特卡洛树按索是阿尔法Go中使用的搜索策略之一。它通过选择具有最大UCB值的子节点来进行搜索。在**多臂赌博机**中，**UCB算法**平衡了利用和探索，选择已经带来最大回报的赌博机，同时也选择那些可能带来更大回报的未被探索的赌博机。蒙特卡洛树搜索包括选择、拓展、模拟和反向传播四个步骤。阿尔法Go通过深度学习和强化学习训练策路网络和价值网络，并结合蒙特卡洛树搜索选择最有潜力的局面。阿尔法GoZero进一步改进了蒙特卡洛树搜索、使用深度残美网络来捕捉围棋局面的模式。
...
> 知识图谱推理是人工智能中的一个重要模型与手段。一阶归纳学习算法通过从**已有的知识图谱中学习**得到推理规则，从而**实现对新的概念、实体和关系的推理**。算法的流程包括将目标位置作为结论，逐步添加前提约束位置并计算信息争议值，选择最优的前提约束位置得到新的推理规则，然后不断重复这个过程直到推理规则不覆盖任何反例。通过这种**归纳学习**的方法，我们可以从大量的数据中学习到普遍化的推理规则，从而不断扩充知识图谱。

---

#### 因果推理

> 因果推理是从大量数据出发建立数据的联合分布模型，然后预测在条件改变后的新联合分布模型中某一方面的结果是否会发生改变。**因果推理可以分为可观测问题、决策行动问题和反事实问题**。为了进行因果推理，可以采用**结构因果模型和因果图两种模型**。结构因果模型是根据数据生成机制建立的模型，而因果图则描述了变量之间的依赖关系。在因果推理中，还**引入了do算子**，用于**干预**系统中的某个变量。因果推理的计算可以通过**调整公式**来实现。此外，还介绍了因果图的不足之外和其他因果推理方法，如反事实推理。逻辑与推理是人工智能领域的重要方面，归纳推理、演绎推理和因果推理是其中的主要方法。

- 结构因果模型
- 因果图
- 因果效应
- do算子
  - do(x)=a表示将DAG中指向节点x的有向边全部切断，并且将x的值固定为常数a
  - do算子的意思可理解为"干预"(intervention)
...
- 训练集中一共有n个标注数据，第i个标注数据记为(x~i~,y~i~),其中第i个**样本数据为xi**，**y是x的标注信息**。
  从训练数据中学习得到的映射函数记为f，f对x~i~的预测结果记为f(x~i~)。**损失函数就是用来计算x~i~真实值y~i~与预测值f(x~i~)之间差值的函数。**
  很显然，在训练过程中希望映射函数在训练数据集上得到“损失”之和最小，即min∑~i=1~Loss(f(x~i~),y~i~)
- **经验风险和期望风险**
  - 经验：在**训练集**上的损失
  - 期望：在**测试集**上
- **过拟合（学习）与欠拟合（学习）**
  - 过拟合：训练集上表现优秀，测试集表现不好
  - 欠拟合：训练集和测试集表现都不优秀
- **生成模型和判别模型**
  - 判别模型 ： 
    - f（一张照片） -> 人脸  
    - f（人脸｜一张照片） -> 0.99
  - 生成模型： 从数据中学习联合概率分布P（X，Y） 难

##### 线形回归（Linear Regression）

> 在现实生活中，往往需要分析若干变量之间的关系，如碳排放量与气候变暖之间的关系、某一商品广告投入量与该商品销售量之间的关系等，这种**分析不同变量之间存在关系的研究叫回归分析**，刻画不同变量之间关系的模型被称为回归模型。如果这个模型是线性的，则称为线性回归模型。

Regression 衰退，来源于父母身高每增长1个单位，子女平均身高增长0.516个单位，存在1到0.516的衰退
...
- 方法:不可能去统计整个国家中每个人是否观看电视节目、进而算出收视率只能抽样一部分人口，然后将抽样人口中观看该电视节目的比例作为该电视节目的全国收视率。
- 霍夫丁不等式:全国人口中看该电视节目的人口比例(记作x)与抽样人口中观看该电视节目的人口比例(记作y)满足如下关系:P(|x-y|≥ e)≤2e-2Ne’(N是采样人口总数、ε∈(0,1)是所设定的可容忍误差范围)
- 结论：**当N足够大时，“全国人口中电视节目收视率”与“样本人口中电视节目收视率”差值超过误差范围e的概率非常小。**也就是结果**近似正确（Probably Approximately Correct ，PAC）**

类比机器学习：若干“弱模型”是否可以将这些弱模型组合起来形成一个“强模型“？
...
> 两者均是学习**输入变量和输出变量之间潜在关系模型**，基于学习所得模型将输入变量映射到输出变量。

监督学习分为回归和分类两个类别。

- 在回归分析中，学习得到一个函数将输入变量映射到连续输出空间，如价格和温度等，即**值域**是连续空间。
- 在分类模型中，学习得到一个函数将输入变量映射到离散输出空间，如人脸和汽车等，即输出变量所在的值域是离散空间值域
...
- k个聚类质心，数据将会更具与聚类之心的欧式距离大小，向聚类之心靠近，最后会得到k个簇
- 可迭代聚类
- 限制推出迭代条件：
  1. 达到限制迭代次数
  2. 前后2次迭代，聚类质心变化不大，认为其已经收敛，推出迭代



##### PCA

> Principle Component Analysis主成分分析：类比人脑在处理图像时，并不会对整体每个像素点进行分析，而是会提取出主要实体，进行信息比对。对于机器学习而言，则是需要对数据进行降维，同时保证图像信息实体的不变
...
> 挖掘单词-单词 、单词-文档以及文档-文档的相关语义关系



##### 线形句别分析

> 基于监督学习的降维方式，也被称为飞雪线形区别分析
...
LLM 生成是具有随机性的，在模型的顶层通过选取不同预测概率的预测结果来生成最后的结果。我们一般可以通过控制 temperature 参数来控制 LLM生成结果的随机性与创造性。

Temperature 一般取值在 **0~1 之间**，当取值较低**接近 0 时**，预测的**随机性会较低**，产生更保守、可预测的文本，不太可能生成意想不到或不寻常的词。当取值较高**接近 1 时**，预测的**随机性会较高**，所有词被选择的可能性更大，会产生更有创意、多样化的文本，有可能生成不寻常或意想不到的词。



###### System Prompt

System Prompt 是随着 ChatGPT API 开放并逐步得到大量使用的一个新兴概念，事实
上，它并不在大模型本身训练中得到体现，而是大模型服务方为提升用户体验所设置的一种策略。

具体来说，在使用 ChatGPT API 时，你可以设置两种 Prompt：一种是 **System Prompt**，该
种 Prompt 内容会在**整个会话过程中持久地影响模型的回复**，且相比于普通Prompt 具有更高的重要性；另一种是 **User Prompt**，这更**偏向于我们平时提到的Prompt，即需要模型做出回复的输入。**

> [!WARNING]
>
> 使用分隔符尤其需要注意的是要防止**提示词注入（**Prompt Rejection**）** 。什
>
> 么是提示词注入？
>
> 就是用户输入的文本可能包含与你的预设 Prompt 相冲突的内容，如果不加分隔，
>
> 这些输入就可能“注入”并操纵语言模型，轻则导致模型产生毫无关联的不正确的输
>
> 出，严重的话可能造成应用的安全风险。
...
- model： 调用的模型
- messages：content ：输出的内容
- max_tokens 最大token数 = prompt tokens + completion tokens
- stream：是否使用流式传输
...
- system ： 负责设定行为框架和长期控制
- User
- Assistant（助手角色）：用于控制模型生成的回复内容



### LangChain

> `python` 统一各大模型API 集成开发库
...
- 只需要继承到顶级父类 Runnable 就可以放在chain上 通过invoke调用
- 字典dict 也可以
...
- **定义**：BaseChatMessageHistory 是 LangChain 中的一个抽象基类，用于管理聊天消息的历史记录。它定义了存储、加载、清空消息等接口，具体实现由子类完成。
- 简单来说就是存在你想要的地方，比如内存、redis、数据库

##### RunnableWithMessageHistory

- **定义**：RunnableWithMessageHistory 是 LangChain 中一个包装类，用于将消息历史管理功能集成到处理链（Runnable）中。它在调用模型前**加载历史消息，调用后保存新消息**，并通过 `session_id` 区分不同会话。
- 简单来说进行时时输入历史信息
...
>**function call 2023年提出**
>
>打通大模型和外部系统的界限，训练大模型调用外部系统的函数，来实现更加强大的功能
>
>存在问题：
>
>1. 需要额外训练大模型，调用function的能力
>2. 没有同意的接口标准
>
>因此出现了MCP **（Model Context Protocol）**
>
>对应DEEPSEEK的接口Tools
