...
- **底层结构**：分块数组 + 中控器（非单个连续数组）。
- **优点**：高效头尾操作、支持随机访问。
- **缺点**：中间插入/删除性能差（需移动元素），迭代器失效规则复杂。

> 注意底层结构，虽然看起来像是双端队列，实际上并不是，谨防迭代器失效问题
...
为了解决虚假唤醒和减少程序员开发成本，重载了wait函数token，返回是否满足条件。若满足执行下一语句，若不满足放弃持有的锁，继续等待。
