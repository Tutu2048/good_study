...
> ```
>
> 注意future对async对影响，如果future定义在局部区域或者没有去承接async返回的future，会导致async阻塞式执行。因为：future拥有着shared_state ，在析构时需要归还堆上的shared_state空间，所以在}想要触发析构，但是空间未被回收，为保证空间安全，则局部futrue会等待async的结束。上述仅仅是影响性能，但是当运用中包含着加锁的操作，就会很隐蔽地导致死锁问题。例如：
>
> ```cpp
> void DeadLock() {
>     std::mutex  mtx;
>     std::cout << "DeadLock begin " << std::endl;
>     std::lock_guard<std::mutex>  dklock(mtx);//上锁
>     {
>         std::future<void> futures = std::async(std::launch::async, [&mtx]() {
>             std::cout << "std::async called " << std::endl;
>             std::lock_guard<std::mutex>  dklock(mtx);//等待上锁
...
-  `set_value()` 设置一个值(运行的子线程 输出中间结果)，由future容器去承接（主线程用future.get拿到中间结果）
   -  猜测设计思路为：子线程对future实例进行更改，保存在一个地方（一般在主线程内，主线程拥有着future实例），等待主线程通过future实例进行get
...
> 处理问题：**业务层**并发语义
>
> 区分：易混淆与proactor、reactor，他俩是用于处理io层面的高效并发，而actor是为了解决业务层需要高并发处理，但是由于资源的共享导致的矛盾
>
> 实现：将业务处理包装成一个类，包含任务队列，完成任务队列中任务。任务提供就是由其他类（任务生成线程）通过actor提供的接口并发地塞入任务队列



#### CSP

> Communicating Sequential Processes，通信顺序进程）是一种**并发编程模型**，其核心思想是**通过通道（Channel）进行消息传递来实现协程/线程间通信**,**通道解耦并发实体，以通信代替共享内存，将并发复杂性转化为消息流设计问题**
>
> 缓冲：解决双方速率不同的问题，保存n个消息
>
> **单向**
...
> Actor和CSP都是并发编程的模型。
>
> Actor更像现实意义的“实体”，是一个具有处理能力的人提供了一个记录任务的册子，需求方在册子上完成登记（各个需求方在记录任务，也就是并发请求的过程，通过条件变量、互斥量保证任务册子的书写顺序和完整），Actor根据册子有任务完成任务（至于工作方式和工作顺序，你不必管，那是每一个actor的“个性”使然），没有任务就挂起；
>
> CSP，正如它的核心Channel名字一样，它更像是一个“工具实体”，双方通过工具进行资源的共享，也就无需去约定的地方（共享区域）去寻找所需资源，而对于channel而言，它并不需要知道这些资源是干什么的，只是无情的搬运机器。这种方式也就是将双方访问共享变量的方式，通过channel的沟通解决掉，双方再去访问资源时，也就不会有冲突了。（还在为做任务每次跑空而苦恼吗？还在为每次都差人一步拿不到资源而哭泣吗？Channel解放你的双腿，解决你的烦恼，[name] Channel你值得拥有🐶）

---

> [!NOTE]
>
> TODO：手搓一个线程池就可以将上面的内容吸收了
...
atomic<int> x;
x.store(0,std::memory_order_relaxed);
x.compare_exchange_strong(expected_val,desired_val);
```

如果expected_val与x的值一致，则用desired_val替换掉x空间内存的内容，返回ture。

如果不一致，则**将expected_val 赋值为当前x的值**，返回false。

---

##### compare_exchange_weak

他不能保证`atomic<T>`的值和`expected`的值相等时也会做交换，很可能原子变量和预期值相等也会返回false，所以使用要多次循环使用

---

**各个线程的组成**

![1697539893049](./picture/1697539893049.jpg)

##### 内存次序

- std::memory_order_relaxed：最为宽松的方式，只保证变量的原则性，并不保证同步性



- memory_order_release，发布操作，store
- memory_order_acquire，获取操作，load
- ~~memory_order_consume ，获取操作，load（约束更弱，C++17 后不再推荐）~~

- memory_order_acq_rel，acquire+release ，load+stroe

  - 上面4个可以实现同步关系-先发布后获取。三者存在**内存屏障**，其效果就是：所有在release操作之前的内存操作（无论是什么内存序），都不会被重排到release操作之后。acquire 则是不会被排到其之前。
  - 注意：虽然存在内存屏障，但是**依旧不保证走到memory层**，如果线程存在同一个bank，它们的同步关系止于Cache层
  ...
  这也就保证了，代码上写**在release前的所有内存操作，都只会在release前执行。代码写在acquire后的操作，都只会在acquire后执行**
  
  - acquire｜consume和release 是**一一配对**的关系


- memory_order_seq_cst：全局一致性，最高限制的方式，不管多少变量，多少线程，都是顺序一致的，存在线程开销（结合上图，理解，就是所有的更改，必须写到memory，其他线程才能进行读写，这样就保证了，变量的更改永远是新的）

> [!CAUTION]
>
> lock 锁是全局可见的，毕竟它只允许一个线程进行进行操作，操作结束后才会释放锁。也就是对临界区的操作结果，一定是被所有线程所看见的



##### 栅栏操作

`atomic_thread_fence(内存次序)`
...
和store +release和acquire +acquire 效果一样，同样也不保证写入memory。但优先推荐使用`y.store(release)`

---

### 无锁并发

我们要实现无锁并发，经常会用到一种结构无锁队列。

结构：环形队列

优势：队列大小固定，队列复用简单，且读写通过head、tail指针pop、push即可；

#### 无锁队列

- 少用一个单位的空间来降低判断逻辑成本（环形队列的常用策略）

  - 判空 ：`head ==tail`

  - 判满：`(tail+1)%max_size == head`
  - head指向第一个元素，tail指向最后一个元素
  - 逻辑层完成出入队操作，无需真的clear空间，比如pop之后，head++后，无需对原先位置进行clear，因为它已经在逻辑上不是第一个元素了，后续push也会将其覆盖掉。
- 用原子操作代替锁的操作
...
> [!WARNING]
>
> 我们学内存次序的原因是为了知道存在重排的存在，了解数据是怎么从软件流向硬件的。但是**永远不要假设**内存序会影响物理内存写入 - 这是硬件的职责。
